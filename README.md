...
- **注册确认密码**（v0.0.3.0）：首次设密 / 注册时需输入两次密码，一致方可完成。
...
- **爆炸保护设置** - 可单独控制领地内是否允许爆炸；全局开关 **`BLOCK_ALL_EXPLOSIONS`** 关闭时，按领地 **`allow_explosion`** 做 **逐方块** 保护（v0.0.8 修复拦截流程，见更新日志）
...
### 🔄 跨服数据同步（v0.0.8）
- **用途**：在多服架构下，通过 TCP 后端将玩家经济、头衔、公会等数据在多个 ARC Core 实例之间保持一致，替代或补充原先仅靠 **共享 SQLite 文件路径**（如 `PLAYER_DATABASE_PATH`、`GUILD_DATABASE_PATH`）的方式
- **模块**：`sync_protocol.py`（消息类型、可同步表枚举、长度前缀 + JSON 编解码）、`sync_server.py`（后端服务端）
- **服务器端模式（已实现）**：在 `core_setting.yml` 中设置 **`ENABLE_SYNC_SERVER=True`**，插件启动后监听 **`SYNC_SERVER_PORT`**（默认 `19999`）；可选 **`SYNC_SERVER_AUTH_KEY`** 要求客户端连接时提供相同密钥。多线程架构：主线程 accept，每客户端独立处理线程；**60 秒** 心跳超时断开；数据变更可 **广播推送** 给已连接客户端（除发起变更方）
- **可同步数据表**：`player_basic_info`、`player_economy`、`player_title`、`title_definitions`、`player_title_unlock_time`、`player_title_equipped`、`guilds`、`guild_members`、`guild_invites`
- **协议能力**：认证、单条/批量增删改查、全量同步、心跳与状态查询、服务端推送通知（`PUSH_NOTIFY`）等
- **客户端模式（配置预留）**：`core_setting.yml` 已提供 **`ENABLE_SYNC_CLIENT`**、`SYNC_SERVER_IP`、`SYNC_CLIENT_SERVER_ID`、`SYNC_CLIENT_SERVER_NAME`、`SYNC_CLIENT_AUTH_KEY` 等项；**客户端连接模块尚未随本版发布**，服主可先在一台机器上启用服务器端模式，待后续版本接入各游戏服
- **与旧方案关系**：仍可使用 `PLAYER_DATABASE_PATH` / `GUILD_DATABASE_PATH` 等指向同一文件；跨服同步服务适合需要 **实时推送** 或 **网络分离的多机部署** 场景
...
│   ├── sync_protocol.py         # 跨服同步协议（v0.0.8+）
│   ├── sync_server.py           # 跨服同步后端服务端（v0.0.8+）
...
- ✅ **跨服数据同步（服务器端）**：新增 **`sync_protocol.py`**、**`sync_server.py`**；配置 **`ENABLE_SYNC_SERVER`**、**`SYNC_SERVER_PORT`**、**`SYNC_SERVER_AUTH_KEY`**。启用后插件内嵌 TCP 同步服务，支持多客户端连接、可选密钥认证、心跳保活与数据变更广播；可同步玩家基础信息、经济、头衔及公会相关表。`core_setting.yml` 与 README 已补充 **客户端模式** 配置项（**`ENABLE_SYNC_CLIENT`** 等），客户端连接实现留待后续版本。详见上文「跨服数据同步」
- ✅ **爆炸监听修复**：修复 **`ActorExplodeEvent`** 在 **`BLOCK_ALL_EXPLOSIONS=False`** 时按领地保护的流程错误。改为用 **`block.x/y/z`** 直接取坐标（避免 `Location` 构造问题）；对需保留的方块通过 **`get_block_at`** 重建 **`block_list`** 再写回，降低直接赋值原 Block 引用导致崩溃的风险；`block_list` 为 `None` 或不可迭代时安全跳过；写回失败时 **回退为取消整次爆炸**，确保禁止爆炸的领地不被破坏

### v0.0.7.6

- ✅ **全局爆炸拦截**：新增配置 **`BLOCK_ALL_EXPLOSIONS`**（默认 `True`）。开启时取消一切 `ActorExplodeEvent`（苦力怕、TNT 等）；关闭后仍按领地 **`allow_explosion`** 做局部保护（v0.0.8 进一步修复逐方块过滤流程）。配置缺失时按默认开启处理。
- ✅ **进服体验**：已移除配置项 **`FORCE_LOGIN`**。每位玩家进服后 **固定自动弹出主菜单一次**（短暂延迟），与设置无关；关闭表单即可继续游玩，敏感操作仍按 **会话内密码验证**（见上文「玩家管理系统」）。**敏感操作**（转账、创建/管理领地、公会创建等）在执行前 **按需密码验证**，**同一会话内验证成功一次即可**。
- ✅ **我的信息 · 修改密码**：主菜单 **我的信息** →「修改密码」；已设密需验 **当前密码** 后设新密；未设密走与注册相同的设密表单，关闭或成功后回到 **我的信息**；改密后清除本会话敏感操作验证状态。详见上文「玩家管理系统」；语言键 **`CHANGE_PASSWORD_*`**（`dist/ARCCore/ZH-CN.txt`）
