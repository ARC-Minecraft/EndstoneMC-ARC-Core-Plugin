...
### 🎯 成就系统（v0.0.4.0）
- **统计**：击杀总数、按生物类型击杀数；破坏方块总数、按方块类型破坏数（SQLite 持久化）
- **条件**：OP 在 **OP 面板 → 成就管理** 中配置「统计键 + 达成数量 → 解锁头衔」；统计键示例：`kill_total`、`kill:minecraft:zombie`、`block_break_total`、`block_break:minecraft:stone`
- **联动**：达成后调用头衔解锁，并发放该头衔在头衔定义中配置的解锁奖励（金钱/物品）
...
- **默认头衔** - 配置 `DEFAULT_TITLE`（逗号分隔），全服玩家拥有；默认稀有度为普通，介绍与奖励为空，OP 可在头衔属性管理中修改
...
│   ├── AchievementSystem.py   # 成就系统（v0.0.4.0+）
...
- ✅ **成就系统**
  - 新模块 `AchievementSystem.py`：击杀/破坏统计、成就条件表、达成记录；OP 面板「成就管理」可配置条件并联动头衔解锁与奖励
- ✅ **财富榜首富头衔**
