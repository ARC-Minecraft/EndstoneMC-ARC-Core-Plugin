...
- ✅ **领地展示框权限**
  - 领地表新增布尔 `allow_frame`，默认 False
  - 为 False 时，在已开放方块互动或方块破坏的前提下，若交互/破坏的方块为 `minecraft:frame` 或 `minecraft:glow_frame` 则拦截，防止他人从领地展示框取物或破坏展示框
  - 领地详情与 OP 公共领地设置中均可配置「展示框权限设置」

- 🔧 **领地数据库**
  - 移除已废弃的 `allow_sub_land` 键；公共领地是否允许私人圈地由 `allow_non_public_land` 控制

- ✅ **头衔管理 UI 优化**
  - 玩家信息→头衔管理中的头衔按钮改为两行：第一行头衔名称（按稀有度颜色 + 加粗），第二行头衔介绍（若有，白色细体）
  - 当前佩戴的头衔显示绿色前缀「【当前】」

- ✅ **玩家头顶名（name_tag）**
  - 进服、头衔变动、玩家重生时均将玩家 `name_tag` 设为 `[头衔]名字` 或 `名字`，便于在游戏内直接看到头衔

- ✅ **生物名称翻译**
  - 新增 `EntityDisplayNameManager` 与配置文件 `entity_display_name.txt`（位于插件目录，与语言文件同目录）
  - 当死亡播报等处的生物名称含有 `:`（如 `entity.ns_ab:vfx_dragon_fire.name`、`entity.dk:mercenary.name`）时，视为 MC 未提供对应翻译，从该文件按「原始名称=显示名」读取用户配置的显示名
  - 未在文件中配置的键会自动追加到文件末尾，等待用户补写翻译

### v0.0.3.0

- ✅ **强制登录**
  - 配置项 `FORCE_LOGIN`：为 true 时，玩家进服强制打开 /arc 菜单进行登录或注册
  - 若未输入正确密码就关闭登录/注册页，会再次执行 /arc 逻辑强制打开

- ✅ **注册确认密码**
  - 注册时需输入两次密码，一致方可完成注册；不一致时提示并重新打开注册页

- ✅ **OP 快捷指令**
  - `/pos1`：记录当前坐标为坐标 1（OP 专属，对应 OP 面板「记录坐标 1」）
  - `/pos2`：记录当前坐标为坐标 2 并打开 OP 面板（OP 专属）
  - `/arc op`：直接打开 OP 面板（OP 专属）；命令用法已写为 `["/arc", "/arc op"]` 以支持多级命令

- ✅ **OP 面板调试模式**
  - OP 面板新增「开启/关闭调试模式」按钮
  - 开启后，当该 OP 触发方块破坏、方块放置、方块交互、生物攻击、生物交互时，会向该玩家发送聊天调试消息，包含事件类型、目标、维度、位置

- ✅ **头衔系统**
  - 新增模块 `TitleSystem.py`：默认头衔（`DEFAULT_TITLE`，逗号分隔）、OP 专属头衔（`OP_TITLE`，单个）；表 `title_definitions`（头衔名、稀有度、介绍、解锁奖励金钱/物品）、`player_title_unlock_time`（xuid、头衔、解锁时间）
  - 聊天展示：远古 QQ 风格，首行 `[头衔]玩家名(年.月.日-时:分)：` 下一行消息；「玩家」前缀可配置（语言键 `TITLE_CHAT_PLAYER_PREFIX`）；`[头衔]玩家名` 加粗且按稀有度上色（普通白 §f、稀有蓝 §9、史诗紫 §d、传奇橙 §6、神话红 §c）
  - 头衔属性：稀有度（普通/稀有/史诗/传奇/神话）、头衔介绍、解锁时间（解锁时记录；默认头衔在首次进服或首次获得时记录；已进服但尚无默认头衔的玩家在下次进服时补发并记录）、解锁奖励（金钱 + 物品列表）
  - 主菜单「我的信息」→「头衔管理」：选择佩戴/不佩戴
  - 进服时：确保默认头衔与 OP 头衔有解锁记录（无则插入并记录时间）；非 OP 若佩戴 OP 头衔则自动解除
  - OP 面板→「头衔管理」：**头衔属性管理**（列出所有头衔，点击编辑稀有度/介绍/解锁奖励）、**创建新头衔**（名称、稀有度、介绍、奖励金钱、奖励物品格式 `物品ID 数量; ...`）、**给所有玩家添加头衔**（选择已有头衔，为当前数据库内所有玩家解锁，仅对现有玩家生效）、**给玩家单独添加头衔**（先输入玩家名，再选择头衔；若该玩家在线则发放解锁奖励）
  - 解锁奖励：解锁时若玩家在线则发放该头衔定义的金钱与物品（`give` 指令）
  - 对外 API：`plugin.api_unlock_title(player, title: str)` 为玩家解锁头衔并发放解锁奖励
...
- ✅ **公共领地系统** - 可设为系统公共领地
  ...
  - OP 可在公共领地详情中进入「公共领地设置」，单独配置：
    - 开放方块互动、开放爆炸、开放生物互动、展示框权限、开放生物伤害
  - 公共领地支持 OP 修改领地名称
...
- ✅ **随机传送系统** - 全新的探险功能
  - 可配置的随机传送中心点和半径
  - 在指定范围内随机生成传送坐标
  - 自动附加羽落（缓降）效果，防止玩家摔死（**v0.0.4.1 起时长为 30 秒**，此前为 10 秒）
...
- ✅ **领地高级设置**（爆炸保护、方块互动开放设置、生物保护系统、展示框权限设置）
- ✅ 传送系统（Home、Warp、TPA、死亡回归、随机传送）
- ✅ **随机传送系统**（可配置中心点和半径、自动缓降/羽落效果，**30 秒**）
