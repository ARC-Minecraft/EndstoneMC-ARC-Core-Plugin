...
[![Version](https://img.shields.io/badge/version-v0.8.14-blue)](https://github.com/ARC-Minecraft/EndstoneMC-ARC-Core-Plugin)
...
- **公共领地「拦截生物生成」** - 全局配置 **`PUBLIC_LAND_BLOCK_ACTOR_SPAWN_MODE`**（默认 **False**）：**False/off** 时各公共领地显示「不开启拦截」且不可开关；**blacklist** / **whitelist** 时每个领地可单独开关拦截。配置 **`PUBLIC_LAND_BLOCK_ACTOR_SPAWN_LIST`**（逗号分隔实体 ID）：白名单=名单上的不拦截，黑名单=只拦截名单上的。领地开关开启后通过 `ActorSpawnEvent` 取消该公共领地内**除玩家外**的匹配实体生成（含模组生物）；数据库字段 `block_actor_spawn`
- **领地移交功能** - 可将领地转移给其他玩家
...
# 公共领地拦截生物生成 (v0.8.10)
# 模式：False=关闭（各领地显示不开启拦截，不可开关）；blacklist=黑名单；whitelist=白名单
PUBLIC_LAND_BLOCK_ACTOR_SPAWN_MODE=False
...
| `api_get_land_info` | `land_id` | `dict`：领地详情；不存在为 `{}`。常见键：`land_name`，`dimension`，`min_/max_x/y/z`，`tp_x/y/z`，`shared_users`，`owner_xuid`，`for_sale`，`sale_price`，各类开关，`public_priority`，`block_actor_spawn`（该领地是否开启拦截；全局模式为 False 时不生效），`owner_paid_money` |
