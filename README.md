...
- **展示框权限设置** - 可禁止领地对展示框/发光展示框的互动与破坏（默认禁止，防止他人从展示框取物）
...
### 🎯 成就系统（v0.0.5.0）
- **新增**：完整的成就模块（`AchievementSystem.py`、`achievement_conditions.py`），与 **OP 面板 → 成就管理** 联动；达成条件后可解锁头衔并发放头衔定义中的解锁奖励（金钱/物品）
- **可配置的击杀成就**：服主可配置「击杀哪些生物、累计多少、解锁哪个头衔」——支持 **单类生物**（`kill_entity`，含 `target_id` 为 `*` 表示任意生物总击杀）、**多类生物击杀数相加**（`kill_entity_sum`）；统计随玩家击杀非玩家实体实时累加
- **数据**：击杀类计数存表 **`player_achievement_stats`**（`stat_key` 如 `kill_total`、`kill:minecraft:zombie`）；达成记录 **`player_achievement_unlocked`**；定义文件 **`plugins/ARCCore/achievements.json`**
- **默认击杀成就包**：OP 可一键写入内置击杀类头衔与条件（与 `默认成就.md` / 内置表一致），便于快速开服
...
- **跨服传送（v0.0.6.0）** - 新增 `/connecttoserver <server_name: str>` 命令，可按已配置目标服务器名称执行跨服传送
...
- **领地管理**：管理所有领地、管理脚下领地、重建领地区块映射（返回统一回到领地管理子菜单）
- **传送管理**：**管理公共传送点**（创建/删除 Warp）；**传送参数配置**（`MAX_PLAYER_HOME_NUM`、随机传送开关/中心/半径、各类传送费用等，与 `core_setting.yml` 传送段一致）
- 邀请奖励配置、**签到配置**（v0.0.4.2：总览展示当前存款/随机条数区间/奖励条目数；**配置存款与随机条数** 弹窗表单；**配置物品奖励列表** 支持按条目进入编辑/删除与新增）、头衔管理、成就管理
...
- **默认头衔** - 配置 `DEFAULT_TITLE`（逗号分隔），**进服时**为每位玩家写入解锁记录（与成就无关）；默认稀有度为普通，介绍与奖励为空，OP 可在头衔属性管理中修改。**请勿**把需通过成就解锁的头衔列入 `DEFAULT_TITLE`（v0.0.5.0 起应用默认击杀成就包时也不再自动合并进此项）
...
| `/connecttoserver` | 按服务器名称执行跨服传送 | 默认 | `/connecttoserver <server_name: str>` |
...
- **领地系统**: 
  ...
  - 在领地详情中可设置爆炸保护、方块互动开放、展示框权限等高级选项
...
- **传送点**: 私人传送点、公共传送点坐标信息
- **成就（v0.0.5.0 说明）**: `player_achievement_stats`（成就用统计计数）、`player_achievement_unlocked`（按头衔名的达成记录）；定义以 **`plugins/ARCCore/achievements.json`** 为准
- **服务器配置**: 出生点坐标、系统设置
...
- ✅ **跨服传送**：新增 `/connecttoserver <server_name: str>` 命令，支持按名称匹配已配置跨服目标并执行传送
